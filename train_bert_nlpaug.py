...
def main(args):
    ...
    # Create an augmenter
    if args.diverse_augment == 'step' or args.diverse_augment == 'value':
        synonym_augmenter_extra = naw.SynonymAug(aug_src='wordnet', aug_p=0.2) 
        swap_augmenter_extra = naw.RandomWordAug(action="swap", aug_p=0.2)
        crop_augmenter_extra = naw.RandomWordAug(action='crop', aug_p=0.2)
        augmenters_extra = [synonym_augmenter_extra, swap_augmenter_extra, crop_augmenter_extra]
        
    synonym_augmenter = naw.SynonymAug(aug_src='wordnet', aug_p=0.1) 
    swap_augmenter = naw.RandomWordAug(action="swap", aug_p=0.1)
    crop_augmenter = naw.RandomWordAug(action='crop', aug_p=0.1)
    augmenters = [synonym_augmenter, swap_augmenter, crop_augmenter]


    def prepare_dataset(args, data, augmenters=None):
        dataset = {
            "text": [],
            "label": []
        }
        for item in data:
            augmented_text = item["text"]
            if augmenters:
                for augmenter in augmenters:
                    try:
                        augmented_text = augmenter.augment(augmented_text)
                    except NotImplementedError as e:
                        print(f"Error with augmenter {augmenter}: {e}")
                        continue  # Skip the augmenter that caused the issue
            augmented_text = augmented_text[0]
            dataset["text"].append(augmented_text)
            dataset["label"].append(item[args.learning_target] * args.scale)
        return dataset

    def prepare_dataset_extra(args, data, augmenters=None, augmenters_extra=None):
        dataset = {
            "text": [],
            "label": []
        }
        for item in data:
            augmented_text = item["text"]
            label_value = item[args.learning_target]

            use_extra = False
            if args.diverse_augment == 'step':
                if 'STEP 1' not in augmented_text:
                    use_extra = True # Augment step 2 and 3
            elif args.diverse_augment == 'value':
                if args.learning_target == 'Difficulty':
                    if label_value > 0.8:
                        use_extra = True
                elif args.learning_target == 'Response_Time':
                    if label_value > 120:
                        use_extra = True

            if use_extra:
                for augmenter in augmenters_extra:
                    try:
                        augmented_text = augmenter.augment(augmented_text)
                    except NotImplementedError as e:
                        print(f"Error with augmenter {augmenter}: {e}")
    ...
    if args.diverse_augment == 'step' or args.diverse_augment == 'value':
        data_train = prepare_dataset_extra(args, data_train, augmenters=augmenters, augmenters_extra=augmenters_extra)
    else:  
        data_train = prepare_dataset(args, data_train, augmenters=augmenters)
    # print('==================================\n',data_train)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--diverse_augment', type=str, default='none', help="none, step, value")
